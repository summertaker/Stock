...
public class NewsListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public NewsListAdapter(Context context, OnLoadMoreListener onLoadMoreListener, ArrayList<News> newsList) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ItemViewHolder) {
            ...
            // 경과일
            //String elapsedText = news.getElapsedText();
            //((ItemViewHolder) holder).tvElapsed.setText(elapsedText);

            // 발행일
            //String publishedText = news.getPublishedText(); // 0000-00-00 00:00:00
            //publishedText = publishedText.replace("-", ".");
            //publishedText = publishedText.substring(6, 16); // 월.일 시:분
            //((ItemViewHolder) holder).tvPublished.setText(publishedText);
        }
    }
